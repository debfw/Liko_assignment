...
export function detectRegion(lat: number, lng: number, country?: string): Region {
  if (country) {
    ...
    if (['brazil', 'argentina', 'chile', 'peru', 'colombia'].includes(countryLower)) {
      return 'south-america';
    }
    
    if (['united kingdom', 'germany', 'france', 'italy', 'spain', 'netherlands', 'belgium', 'sweden', 'norway'].includes(countryLower)) {
      return 'europe';
    }
    
    if (['egypt', 'south africa', 'nigeria', 'kenya', 'morocco', 'ethiopia'].includes(countryLower)) {
      return 'africa';
    }
    
    if (['china', 'japan', 'india', 'south korea', 'indonesia', 'thailand', 'vietnam', 'singapore'].includes(countryLower)) {
      return 'asia';
    }
...
export function getNodeSize(population?: number): number {
  ...
  if (population > 20000000) return 20;
  if (population > 10000000) return 15;
  return 10;
}